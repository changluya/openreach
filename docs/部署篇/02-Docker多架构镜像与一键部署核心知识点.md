...
```bash
docker build -t codercl/openreach:1.0.2 .
```
...
```bash
docker buildx build   --platform linux/amd64,linux/arm64   -t codercl/openreach:1.0.2   -t codercl/openreach:latest   --push   .
```
...
```bash
docker run codercl/openreach:1.0.2
```
